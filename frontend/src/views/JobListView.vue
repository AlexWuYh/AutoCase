<template>
  <div class="jobs">
    <el-card>
      <template #header>
        <div class="card-header"><span>生成任务</span></div>
      </template>
      ...
      <el-table v-loading="loading" :data="rows" stripe empty-text="暂无任务">
        <el-table-column prop="id" label="ID" width="60" />
        <el-table-column prop="group_name" label="需求集" min-width="140" />
        <el-table-column label="状态" width="100">
          <template #default="{ row }"><StatusTag :status="row.status" /></template>
        </el-table-column>
        <el-table-column label="进度" width="160">
          <template #default="{ row }">
            <el-progress :percentage="row.percent" :stroke-width="14" :status="row.status === 'failed' ? 'exception' : (row.status === 'success' ? 'success' : undefined)" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 'success'" size="small" type="success" @click="onExport(row, 'xlsx')">导出</el-button>
    ...
    <el-drawer v-model="drawerOpen" title="任务详情" size="70%">
      <template v-if="detail">
        <el-descriptions :column="3" border style="margin-bottom:16px">
          ...
          <el-descriptions-item label="LLM 配置">{{ detail.llm_config_name || '-' }}</el-descriptions-item>
          <el-descriptions-item label="Prompt">{{ detail.prompt_name || '-' }}</el-descriptions-item>
          <el-descriptions-item label="创建时间">{{ fmt(detail.created_at) }}</el-descriptions-item>
        </el-descriptions>
        <el-alert v-if="detail.error" type="error" :title="detail.error" style="margin-bottom:12px" />
        <el-progress :percentage="detail.percent" :status="detail.status==='failed'?'exception':(detail.status==='success'?'success':undefined)" style="margin-bottom:16px" />

        <el-table v-loading="casesLoading" :data="caseRows" stripe max-height="50vh" empty-text="暂无用例">
        ...
        <div style="margin-top:12px; display:flex; gap:8px; justify-content:flex-end">
          <el-button v-if="detail.status==='success'" type="success" @click="onExportDetail('xlsx')">导出 Excel</el-button>
          <el-button v-if="detail.status==='success'" @click="onExportDetail('csv')">导出 CSV</el-button>
...
async function loadCases() {
  ...
  try {
    ...
    caseRows.value = data.items; caseTotal.value = data.total
...
function onExport(row: GenerationJob, fmt: 'xlsx' | 'csv') {
  const token = localStorage.getItem('access_token')
  window.open(`${jobsApi.exportUrl(row.id, fmt)}&token=${token}`, '_blank')
}
